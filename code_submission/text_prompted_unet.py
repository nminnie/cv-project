...
class CLIPPromptedUNet(nn.Module):
    ...
    def forward(self, x, text_embeddings):
        ...
        Args:
            ...
            text_embeddings: CLIP text embeddings of shape (B, embed_dim)
                             representing a single prompt per image
        """
        batch_size = x.shape[0]
        
        # Process text embeddings - ensure they are 2D [batch_size, embed_dim]
        if text_embeddings.dim() > 2:
            # If we have a higher dimension, flatten to 2D
            text_embeddings = text_embeddings.view(batch_size, -1)
...
def get_text_features(clip_model, prompts, device):
    """
    Get text features for each prompt
    
    Args:
        clip_model: CLIP model
        prompts: List of single prompts
        device: torch device
...
def train_text_prompted_model(model, train_loader, val_loader, clip_model, run_path, num_epochs=20, cat_weight=2.1, device=device):
    """
    ...
    Args:
        ...
        clip_model: CLIP model for encoding text prompts
    ...
    try:
        ...
        # Test forward pass with random prompt selection
        print("Testing forward pass...")
        with torch.no_grad():
            batch_size = test_imgs.shape[0]
            # Generate random prompts for the batch
            random_prompts = []
            for i in range(batch_size):
                # Get the dominant class in the mask (ignoring 255)
                mask = test_masks[i]
                valid_pixels = mask[mask != 255]
                if len(valid_pixels) > 0:
                    values, counts = torch.unique(valid_pixels, return_counts=True)
                    dominant_class = values[torch.argmax(counts)].item()
                else:
                    dominant_class = 0  # Default to background
                
                # Select a random prompt for the dominant class
                prompt_options = class_prompts[int(dominant_class)]
                random_prompt = random.choice(prompt_options)
                random_prompts.append(random_prompt)
            
            # Encode the prompts with CLIP
            batch_text_features = get_text_features(clip_model, random_prompts, device)
    ...
    for epoch in range(num_epochs):
        ...
        for images, masks in train_pbar:
            ...
            # Generate appropriate prompts for each image in the batch
            ...
            # Encode the prompts with CLIP
            batch_text_features = get_text_features(clip_model, batch_prompts, device)
            batch_text_features = batch_text_features.to(next(model.parameters()).dtype)
...
def evaluate_model(model, dataloader, clip_model, device):
    """
...
if __name__ == "__main__":
    ...
    val_results = evaluate_model(model, val_loader, clip_model, device)

    print("\nEvaluating model on test set:")
    test_results = evaluate_model(model, test_loader, clip_model, device)
